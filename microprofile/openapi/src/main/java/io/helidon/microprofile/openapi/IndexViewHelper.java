...
/**
 * Helper for preparing index views for annotation scanning used by the {@link io.helidon.microprofile.openapi.MpOpenApiFeature}
 * class.
 * <p>
 *     The SmallRye OpenAPI layer accepts a single Jandex {@link org.jboss.jandex.IndexView} index with which it scans for
 *     OpenAPI-related annotations and then augments the OpenAPI in-memory model it is building. To provide some
 *     consistency in how Helidon's OpenAPI output looks when multiple {@link jakarta.ws.rs.core.Application} objects are present
 *     and when there are types in the index decorated with OpenAPI-related annotations but not used by any
 *     given application. Helidon creates a separate index view for each {@code Application} and submits each view to
 *     SmallRye for it to augment the in-memory model.
 * <p>
 *     We accomplish this partly by using a different SmallRye {@code OpenApiConfig} object for each
 *     {@code Application}, tailored to limit the indexed types SmallRye processes for that application.
 *
 *
 * </p>
 */
class IndexViewHelper {
    ...
    private static final System.Logger LOGGER = System.getLogger(IndexViewHelper.class.getName());

    private final HelidonOpenApiConfig openApiConfig;
    private final Supplier<? extends IndexView> singleIndexViewSupplier;
    private final boolean useJaxRsSemantics;

    IndexViewHelper(HelidonOpenApiConfig openApiConfig,
                    Supplier<? extends IndexView> singleIndexViewSupplier,
     ...
     */
    FilteredIndexView filteredIndexView(FilteredIndexView viewFilteredByConfig,
            ...
            if (LOGGER.isLoggable(System.Logger.Level.DEBUG)) {
                LOGGER.log(System.Logger.Level.DEBUG, String.format("""
                        ...
                        has been turned off for OpenAPI processing using %s""",
                                                                    MpOpenApiFeature.Builder.USE_JAXRS_SEMANTICS_FULL_CONFIG_KEY,
                                                                    appClassName));
            }
            return viewFilteredByConfig;
        }
        ...
        FilteredIndexView result = filteredIndexView(viewFilteredByConfig, excludeClasses);
        if (LOGGER.isLoggable(System.Logger.Level.DEBUG)) {
    ...
    private FilteredIndexView filteredIndexView(FilteredIndexView viewFilteredByConfig, Set<String> excludeClasses) {

        HelidonMpOpenApiConfig mpOpenApiConfigExcludingSpecifiedClasses = HelidonMpOpenApiConfig.builder()
                .scanExcludeClasses(excludeClasses)
                .build();
        return new FilteredIndexView(viewFilteredByConfig, mpOpenApiConfigExcludingSpecifiedClasses.openApiConfig());
    }
    ...
    private static Set<String> classNamesToIgnore(List<JaxRsApplication> jaxRsApplications,
        ...
        Set<String> result = // Start with all other JAX-RS app names.
                jaxRsApplications.stream()
                        .map(IndexViewHelper::toClassName)
     ...
     */
    List<FilteredIndexView> perAppFilteredIndexViews() {
        ...
        FilteredIndexView viewFilteredByConfig = new FilteredIndexView(indexView, openApiConfig.openApiConfig());
    ...
    private static boolean hasImplementationOrIsIncluded(IndexView indexView, ClassInfo classInfo) {
        ...
        return !Modifier.isInterface(classInfo.flags())
                ...
                .anyMatch(IndexViewHelper::isConcrete);
    }
