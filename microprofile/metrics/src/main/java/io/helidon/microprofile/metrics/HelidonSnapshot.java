...
class HelidonSnapshot extends Snapshot {
    ...
    public HistogramBucket[] bucketValues() {
        return StreamSupport.stream(delegate.bucketValues().spliterator(), false)
                .map(countAtBucket -> new HistogramBucket(countAtBucket.bucket(), (long) countAtBucket.count()))
