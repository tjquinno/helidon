...
* *Blueprint*: The generated code extending the blueprint. It is part of the public API and includes the fluent builder implementation and static factory methods.
...
| `RuntimeType.PrototypedBy`  | Yes      | Annotation on runtime type that is created from a `Blueprint`, to map it to the prototype it can be created from, used to trigger annotation processor for validation
