...
:rootdir: {docdir}/..
:health-inc: {rootdir}/includes/health.adoc
:default-endpoint-prefix: /observe
...
include::{health-inc}[tag=health-status-codes]
...
include::{health-inc}[tag=builtin-health-checks-intro]
...
include::{health-inc}[tag=rest-endpoints-intro]
HTTP response status values are the same as described in the <<health-status-codes-table,status codes table>>.

include::{health-inc}[tag=rest-endpoints-wrapup]
...
include::{health-inc}[tag=configuring-behavior]
include::{health-inc}[tag=configure-endpoint]
