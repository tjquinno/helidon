...
----
so your service complies with MicroProfile Metrics 5.1. See the <<controlling-gc-time,longer discussion below>> in the  Configuration section.
====
...
Beginning with Microprofile Metrics 5.1, the built-in `gc.time` metric is checked to be a gauge; the TCK formerly checked for a counter.
To comply with the earlier TCK, previous releases of Helidon 4.x have used a counter for `gc.time` and--to preserve backward compatibility for Helidon users--Helidon {helidon-version} continues to do so by default.

Setting `metrics.gc-time-type` as described above to use a gauge for `gc.time` allows your service to comply with MicroProfile Metrics 5.1.

The ability to choose the metric type for `gc.time` is deprecated and is planned for removal in a future major release of Helidon.
